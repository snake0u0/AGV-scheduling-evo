...
0. **Stage 0c 남은 것**: (a)(b) 완료(위 10번). 남은 것:
   **(c) [최우선] 규칙 순위를 600초 x pop1000에서 재측정** - 기존 규칙 결론(공차 벌점 주효과 등)의
   유효성이 여기서 갈림. **(d) 예산 곡선 연장**: 3600초 x pop 3000~5000. 평평해지면 구조 문제가
   되살아나고, 계속 떨어지면 GA로 충분 -> **LNS 판정은 이 결과로.**
   (e) 10초 구간 population 하향 탐색(20/30/50) - 진화 루프 설정용.
...
- **[2026-07-31 추가] pop=70에서 측정한 모든 규칙 비교 결과**: 주 캠페인 / 재현성 / 2x2 ablation /
  교란제거 재평가 / 문헌격차 65%. pop1000에서 격차가 94.5%->24.1%로 바뀌므로 **규칙 간 순위가
  유지되는지 재측정 전까지 인용 주의.** 특히 "공차 벌점이 주효과"(ablation)가 가장 중요한 확인 대상.
