...
0. **우선순위 재편 (2026-08-01 저녁, 위 11번 결과)**:
   ...
   **(B) 규칙이 중요해지는 예산 구간 탐색.** 10/60/600초 x 각 예산 최적 pop에서 규칙 간 차이 크기.
   "규칙은 저예산에서 중요하다"가 서면 논문의 링이 동적/온라인 쪽으로 이동.
   (C) 예산 곡선 3600초 연장 -> LNS 판정. **우선순위 하락**, 규칙 쪽이 급함.
   (D) 10초 구간 population 하향 탐색(20/30/50) - 진화 루프 설정용.
