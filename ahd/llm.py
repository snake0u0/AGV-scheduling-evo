"""Rule proposer for the LLM-AHD loop.

A candidate rule is a *genome*: a dict {feature_name: weight} rendered to a scoring
expression "w1*f1 + w2*f2 + ...". Higher score = higher priority. This linear form
spans the classical rules (NV = {travel_time:-1}, EDD = {slack:-1}, ...) and is valid
by construction, so the loop machinery can be exercised with no LLM/API key.

`MockLLM` stands in for a real LLM: it seeds and varies genomes by local edits and
crossover. A real LLM swaps in by implementing the same two methods
(`seed_population`, `vary`) but returning model-generated (agv_expr, machine_expr)
pairs from EoH/ReEvo-style prompts. See skill `ahd-loop`.
"""
