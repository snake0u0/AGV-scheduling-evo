...
> ⚠️ **정정(후속 발견)**: 이 실행은 claude CLI **도구 접근이 켜진 상태**였습니다. 이후 모델이 규칙을 "제안"하는
> 대신 **스스로 평가기를 돌려 탐색**할 수 있었음을 발견했습니다(오염). **도구를 끈 공정한 재실행 결과는**
> `2026-07-04-demo-presentation.md` **§6**에 있습니다(클린: Sonnet OR3 3.68%). 아래 수치는 오염 가능성이 있는 원기록입니다.
