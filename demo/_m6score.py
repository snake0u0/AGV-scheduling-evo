...
def score(code, label=""):
    ...
    if fn is None:
        print(f"{label:10s}: COMPILE/SMOKE FAIL")
        return
    t = exc(fn, train)
    v = exc(fn, valid)
    te = exc(fn, test)
    ...
    print(f"{label:10s} train={t:6.3f} valid={v:6.3f} test={te:6.3f} FULL={f:6.3f} Weib={w:6.3f}{star}")


if __name__ == "__main__":
    from demo._m6cands import CANDS
    print("baseline v0: FULL=2.957 train=3.545  (FunSearch-OR FULL=3.11)")
