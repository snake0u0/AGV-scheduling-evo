...
- **파서 -> decode -> GA 전 과정이 end-to-end로 동작**한다. 실제 Dauzere 인스턴스에서 GA가 문헌
  Table 8과 **같은 자릿수**의 Cmax를 낸다(이 3개 인스턴스에서 +13~43%, 아래 §3.
  **주의: 이 수치는 전체 격차가 아니다. 33개 전체 실측은 65%이며 `2026-07-29-literature-gap-measurement.md`가 정정한다**).
  우리 GA는 평범한 뼈대이고 문헌은 최대
...
> **[정정 2026-07-29] 아래 표의 격차는 인스턴스 3개(01a/07a/13a) 표본이며 전체를 대표하지 않는다.**
> 이 3개는 기계 유연도가 낮은 축이라 상대적으로 우리가 잘하는 인스턴스였다. held-out 33개 전체 실측
> 격차는 **65%**이고, 유연도가 높은 인스턴스에서는 최대 172%까지 벌어진다.
> 정확한 수치는 `2026-07-29-literature-gap-measurement.md`.
