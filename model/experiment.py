...
def evolve_bundle(proposer, train, pop_size=20, n_gens=6, log=print, record=None,
                  init_pop=None, stop_after_failures=3):
    ...
    `init_pop` starts from a given population instead of the proposer's seeds, so a run
    stopped by a usage limit can be continued from where it left off rather than begun
    again.

    `stop_after_failures` ends the run once that many consecutive generations produce no
    model reply. A proposer that cannot reach the model recycles its parents, so the loop
    would otherwise keep spending generations that evolve nothing and finish looking like
    a completed run. Stopping makes the shortfall visible in the generation count.
    """
    seen = {}
    ...
    start = init_pop if init_pop else proposer.seed_population(pop_size)
    population = [(b, fit(b)) for b in start]
    population.sort(key=lambda x: x[1])
    best = population[0]
    history = [best[1]]
    if record is not None:
        record.append({"gen": 0, "population":
                       snapshot(population, "resumed" if init_pop else "seed")})
    ...
    for g in range(1, n_gens + 1):
        ...
        before = getattr(proposer, "fails", 0)
        kids = proposer.vary(elites, pop_size - n_elite)
        if getattr(proposer, "fails", 0) > before:
            consecutive_failures += 1
            if consecutive_failures >= stop_after_failures:
                log(f"stopping at gen {g}: {consecutive_failures} consecutive generations "
                    f"with no model reply")
                break
        else:
            consecutive_failures = 0
